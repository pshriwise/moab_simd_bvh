...
class BVHTraverser {
 public:
				     ...
				     StackItemT<NodeRef>* stackEnd)
  {
    ...
    static void traverseAll(NodeRef& current_node, 
			    size_t mask,
			    const vfloat4& tNear,
			    StackItemT<NodeRef>*& stackPtr,
			    StackItemT<NodeRef>* stackEnd)
    {
      ...
      if (!node->child(0).isEmpty() ) { stackPtr->ptr = node->child(0); stackPtr->dist = ((unsigned int*)&tNear)[0]; stackPtr++; }
      if (!node->child(1).isEmpty() ) { stackPtr->ptr = node->child(1); stackPtr->dist = ((unsigned int*)&tNear)[1]; stackPtr++; }
      if (!node->child(2).isEmpty() ) { stackPtr->ptr = node->child(2); stackPtr->dist = ((unsigned int*)&tNear)[2]; stackPtr++; }
      if (!node->child(3).isEmpty() ) { stackPtr->ptr = node->child(3); stackPtr->dist = ((unsigned int*)&tNear)[3]; stackPtr++; }
      sort(stackPtr[-1], stackPtr[-2], stackPtr[-3], stackPtr[-4]);
